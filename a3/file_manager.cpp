...
char *PageHandler::GetData() { return data; }

// access method - get access to page number
int PageHandler::GetPageNum() { return pageNum; }
...
PageHandler FileHandler::FirstPage() {
    // logic - use NextPage with page number -1 == same as fetch first valid
    // page
    return NextPage(-1);
}
...
PageHandler FileHandler::LastPage() {
    // logic - use PrevPage with page number = total_pages == same as fetch
    // first valid page from last (total_pages-1)
...
PageHandler FileHandler::PageAt(int page_number) {
    ...
    char *page_in_buffer = bufferManager->GetPage(
        PageDescriptor(this->unix_file_desc, page_number));
    // get page header
    PageHdr *page_hdr = (PageHdr *)page_in_buffer;
    // if slot is not free
    // set page number
    // and set page data to point to starting position of data part in page
    // obtained from buffer manager
...
PageHandler FileHandler::NextPage(int page_number) {
    ...
    for (; page_number <= hdr.totalPages;
         page_number++) { // MR: page_number <= totalpages to ensure that it
                          // runs this at least once if file contains only one
                          // page
        pageHandle = PageAt(page_number);
        // if page retrieved without error, stop checking further
        if (pageHandle.GetPageNum() != -1)
            break;
    }
    return pageHandle;
}
...
PageHandler FileHandler::PrevPage(int page_number) {
    if (!checkPageValid(page_number) &&
        page_number !=
            hdr.totalPages) { // allow totalPages (call from LastPage)
        // return invalid page
        throw InvalidPageException();
    }
...
// allocate a new page in file ( either add at end (if free list empty) or use
// one from the free list) incase of errors - returns INVALID_PAGE
PageHandler FileHandler::NewPage() {
    ...
    if (hdr.firstFreePage != END_FREE) {
        ...
        page_buffer = bufferManager->GetPage(
            PageDescriptor(this->unix_file_desc, page_number));
        // update the head of free page list for file
        hdr.firstFreePage = ((PageHdr *)page_buffer)->nextFreePage;
    } else { // free pages
        page_number =
            hdr.totalPages; // new page will be added at the end of file
        page_buffer = bufferManager->AllocatePage(PageDescriptor(
            this->unix_file_desc,
            page_number)); // allocate and load the page in buffer manager
...
// that page becomes available to be used in future by adding it to the free
// list
...
bool FileHandler::FlushPages() {
    if (this->hdrChanged) {
        ...
        int _wr_res =
            write(this->unix_file_desc, (char *)&hdr, sizeof(FileHdr));
        if (_wr_res < 0)
...
FileHandler FileManager::CreateFile(const char *filename) {
    ...
    int fd = open(filename, O_CREAT | O_EXCL | O_RDWR,
                  0660); // updated to fail if file already exists
    if (fd == -1)
...
FileHandler FileManager::OpenFile(const char *filename) {
    ...
    if (fileHandle.unix_file_desc == -1)
        throw InvalidFileException();
    // read header
...
bool FileManager::CloseFile(FileHandler &fileHandle) {
    ...
    if (!fileHandle.FlushPages())
        return false; // flush pages to file, if error in flushing all pages ,
                      // return false
...
void FileManager::ClearBuffer() { bufferManager->ClearBuffer(); }
// interface to printing buffer contents
void FileManager::PrintBuffer() { bufferManager->PrintBuffer(); }
