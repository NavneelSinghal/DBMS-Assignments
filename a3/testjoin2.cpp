...
bool empty_input1(FileManager &fm) {
    ...
    if (pid == 0) {
        // Run join2
        if (execlp("./join2", "./join2", "join_input1",
                   "TestCases/TC_join2/input2_join2", "output",
                   (char *)NULL) == -1) {
            perror("exec error");
            exit(-1);
        }
...
bool empty_input2(FileManager &fm) {
    ...
    if (pid == 0) {
        // Run join2
        if (execlp("./join2", "./join2", "TestCases/TC_join2/input1_join2",
...
bool full_page(FileManager &fm) {
    ...
    if (pid == 0) {
        // run join2
        if (execlp("./join2", "./join2", "join_input1", "join_input2", "output",
                   (char *)NULL) == -1) {
            perror("exec error");
            exit(-1);
        }
...
bool small_join(FileManager &fm) {
    ...
    for (int i = 0; i < size; i++) {
        input1_nums[nums1[i]]++;
        input2_nums[nums2[i]]++;
        input1.write(nums1[i]);
        input2.write(nums2[i]);
    }
...
bool large_join(FileManager &fm) {
    ...
    if (pid == 0) {
        ...
        for (int i = 0; i < 190000; i++) {
            if(input1_nums.find(1)!=input1_nums.end() || input2_nums.find(1)!=input2_nums.end() )
            continue;
            if (output_nums[i] == input1_nums[i] * input2_nums[i])
                continue;
            else {
